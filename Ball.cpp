...
void init_balls() {
	int x, y;
	ballHandle = LoadGraph("./src/Ball3.png");//変更箇所
	GetGraphSize(ballHandle, &x, &y);
	//printfDx("Ball_X:%d Ball_Y:%d\n", x, y);

	for (int i = 0; i < MAX_BALLS; i++) { 
		balls[i].x	   = FIELD_WIDTH / 2;
		balls[i].y	   = FIELD_HEIGHT - BALL_SHOT_POS_OFFSET;//変更箇所
...
void draw_ball() {
	...
	DrawCircle(FIELD_WIDTH / 2, FIELD_HEIGHT - BALL_SHOT_POS_OFFSET, 16, GetColor(0, 255, 100), true);//変更箇所
	...
	for (int i = 0; i < MAX_BALLS; i++) {
		...
		//マウス座標の表示
		DrawFormatString(FIELD_WIDTH - 110, FIELD_HEIGHT - 100, GetColor(0, 0, 255), "MX:%d\nMY:%d", m_Xpos,m_Ypos);
		//ショットのラジアン値
		DrawFormatString(FIELD_WIDTH - 110, FIELD_HEIGHT - 60, GetColor(0, 0, 255), "ANGLE:%.2lf", angle);
		//角度
		DrawFormatString(FIELD_WIDTH - 110, FIELD_HEIGHT - 40, GetColor(0, 0, 255), "DEG:%d", deg);			
	}
}